...
function generateEquation() {
  ...
  const percentage = config.percentages[state.currentLevel];
  const randomNumber = Math.floor(Math.random() * 999);
  state.solution = parseFloat((randomNumber * percentage / 100).toFixed(2));

  const equationText = `${percentage}% of ${randomNumber} = ?`;
  const latex = `${percentage}\\% \\times ${randomNumber} = ?`;
  state.currentEquation = equationText;
...
function checkAnswer() {
  ...
  const tolerance = 0.005; // For 2 decimal place rounding
  ...
  if (Math.abs(playerAnswer - state.solution) < tolerance) {
    ...
    state.streak = 0;
    showFeedback(elements, `❌ The answer was ${state.solution.toFixed(2)}`, false);
    updateStats(elements, state);
