...
const config = {
  ...
  maxScoresToStore: 50,
  timeLimits: {
    1: 15, // Level 1 time limit
    2: 20, // Level 2 time limit
    3: 25 // Level 3 time limit
  },
  basePoints: {
    1: 15, // Level 1 base points
    2: 20, // Level 2 base points
    3: 25 // Level 3 base points
  }
};

let state = {
  scoreHistory: [],
  highScore: localStorage.getItem('cubesHighScore') || 0,
  score: 0,
  streak: 0,
...
function generateEquation() {
  ...
  switch (state.currentLevel) {
    case 1: // 1-digit cubes (1-9)
      baseNumber = Math.floor(Math.random() * 9) + 1;
      break;
      
    case 2: // 2-digit cubes (10-29)
      baseNumber = Math.floor(Math.random() * 20) + 10; // 10-29 range for manageable cubes
      break;
      
    case 3: // 3-digit cubes (100-999)
      baseNumber = Math.floor(Math.random() * 10) + 10; // Reduced range for practicality
      break;
  }
  
  const cube = baseNumber * baseNumber * baseNumber;
  const latex = `${baseNumber}^3 = ?`;
  
  state.currentEquation = `${baseNumber}³ = ?`;
  state.solution = cube;
...
function trackScore(points, timeTaken, isStreakBonus = false) {
  ...
  if (state.score > state.highScore) {
    ...
    localStorage.setItem('cubesHighScore', state.highScore);
  }
