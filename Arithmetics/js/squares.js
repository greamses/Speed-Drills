...
let state = {
  scoreHistory: [],
  highScore: localStorage.getItem('squaresHighScore') || 0,
  score: 0,
  streak: 0,
...
function generateEquation() {
  ...
  switch (state.currentLevel) {
    case 1: // 1-digit squares
      baseNumber = Math.floor(Math.random() * 9) + 1; // 1-9
      break;
      
    case 2: // 2-digit squares
      baseNumber = Math.floor(Math.random() * 90) + 10; // 10-99
      break;
      
    case 3: // 3-digit squares
      baseNumber = Math.floor(Math.random() * 900) + 100; // 100-999
      break;
  }
  
  const square = baseNumber * baseNumber;
  const latex = `${baseNumber}^2 = ?`;
  
  state.currentEquation = `${baseNumber}² = ?`;
  state.solution = square;
...
function trackScore(points, timeTaken, isStreakBonus = false) {
  ...
  if (state.score > state.highScore) {
    ...
    localStorage.setItem('squaresHighScore', state.highScore);
  }
